...
// GitHub Actions vs Local fallback
...
async function fetchMatchesForRange(dateFrom, dateTo) {
  ...
  try {
    ...
    return matches.map(m => ({
      ...
      source: "football-data.org (github-actions enriched)"
    }));
  } catch (e) {
...
async function run() {
  ...
  if (matches) {
    ...
    for (const [date, events] of Object.entries(grouped)) {
      ...
      if (date === dStr(now)) {
        await fs.collection('matches').doc('today').set({ events, lastUpdated: admin.firestore.FieldValue.serverTimestamp(), source: "github-actions-sync" });
        await db.ref("/live_matches").set({ events, lastUpdated: Date.now(), quotaExceeded: false });
      } else if (date === dStr(yesterday)) {
        await fs.collection('matches').doc('yesterday').set({ events, lastUpdated: admin.firestore.FieldValue.serverTimestamp(), source: "github-actions-sync" });
      } else if (date === dStr(tomorrow)) {
        await fs.collection('matches').doc('tomorrow').set({ events, lastUpdated: admin.firestore.FieldValue.serverTimestamp(), source: "github-actions-sync" });
      }
    }
  }
  ...
  // 3. Update Firestore 'global' field
  try {
